 ...
 * @param {Object} [options.tickIntervalTime] Time in ms between every tick
 * @param {Function} [options.uuidMethod] UUID generator method
 * @param {Function} [options.onThreadStart] Job to run when a thread is spawned
 *
 * @return {Promise}
 */
const start = async (options = {}) => {
  ...
  ctx.onThreadStart = options.onThreadStart;
